...
class Day07 : Puzzle<List<Equation>, Long> {
    ...
    override fun parse(input: List<String>): List<Equation> {
        return input.map { line ->
            "[0-9]+".toRegex().findAll(line).map { it.value.toLong() }.toList()
                .let { i -> Equation(i.first(), i.drop(1)) }
        }
    }
    ...
    private fun evaluate(equation: Equation, includeOr: Boolean = false): Boolean {
        val list = mutableListOf(
            Step(
                amount = equation.amounts.first(),
                index = 1,
                debug = equation.amounts.first().toString()
            )
        )
        while (list.isNotEmpty()) {
            val step =
                list.removeLast() //DFS (removeLast) for part 2: 1.034159458s, BFS (removeFirst) for part 2: 2m 45.341065s
